...
class CNNBranch(nn.Module):
    ...
    def forward(self, h):
        ...
        xISRU = self.ISRU_([ torch.cat((xx1,xx2),dim=1) , torch.cat((yy1,yy2),dim=1) , torch.cat((zz1,zz2),dim=1)  ])#,1e-9)
        ...
        outx += delta0_*torch.ones_like(outx)/12 #channel norm
...
class clothNet(nn.Module):
    def __init__(self,dt_=(4e-2)/128,pressure=0): #can add index and velocity for B.C., and collision setting
        ...
        self.cnncell = pdeFix(dt=dt_) #.cuda() can be called outside
    ...
    def pressureCal(self,xx):
        ...
        #i = roll_i[0]#[ind]
        #j = roll_i[1]
        #rolli1,rolli_1,rollj1,rollj_1 = xx,xx,xx,xx
            #if i == 1:
        ...
        ff=self.crossP(rolli_1,rollj1)+self.crossP(rollj1,rolli1)+self.crossP(rolli1,rollj_1)+self.crossP(rollj_1,rolli_1)
        return ff*self.press
...
#        basicf += self.pressureCal(xx)*self.dt*self.drag
        return self.pressureCal(xx)
        #print(basicf.shape)
#        vout = v*self.drag+basicf
        #print(vout.shape)
        #apply constrain
        #if ball
        # Create two tensors on GPU: e.g.
...
if __name__ == '__main__':
    ...
    p_state =  np.transpose(pdata, (0, 3, 1, 2))
...
#        simulate(simu,in_state,5000)
    ...
    #save_model(model, 'cnn_final.pt', save_path)
